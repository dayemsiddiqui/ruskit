...
pub trait Model: for<'r> FromRow<'r, SqliteRow> + Serialize + DeserializeOwned + Send + Sync + Sized + Unpin + ModelValidation + ValidationRules {
    ...
    /// Define a factory instance of this model with fake data.
    /// 
    /// This method must be implemented for each model to support factory functionality.
    /// Use the `fake` crate's generators to create realistic test data.
    /// 
    /// # Example
    /// ```rust
    /// use fake::{Fake, Faker};
    ...
    ///     Self {
    ///         id: 0, // Always use 0 for id as it will be set by the database
    ///         name: Faker.fake(),
    ///         email: Faker.fake(),
    ...
    /// Create a factory instance and save it to the database
    async fn factory() -> Result<Self, DatabaseError> {
        let instance = Self::factory_definition();
